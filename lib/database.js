...
module.exports = {
  ...
  update_richlist: function(list, cb){
    if (list == 'received') {
      ...
      Address.find({}).sort({balance: 'desc'}).skip(1).limit(100).exec(function(err, addresses){
        Richlist.update({coin: settings.coin}, {
          balance: addresses,
        }, function() {
          return cb();
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      var i = loop.iteration();
      var count = i + 1;
      var percentage = ((richlist.balance[i].balance / 100000000) / (stats.supply-9299999999.99598500)) * 100;
      if (count <= 25 ) {
